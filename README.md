TKoolFacetileMakerFor2000 マニュアル
================================================================================
...
***このソフトはTKoolFacetileMaker2をベースに2000専用に変更をしたバージョンです
が、一部機能を廃止し、仕様を変更しています。***

あと開発環境が変わって、作成時点ではまだWindows環境での動作を確認していません。
多分動くはずですが、ちょっとこのドキュメント作成時点ではまだWinが使えないので。

***注意点として、propertiesというフォルダと、その中の設定ファイルが必須***
という問題があります。

私がどんくさいミスをして過去に自分が作ったプログラムを上書き消去してしまっていて
プログラムの修正が難しかったがためです。

簡単に言うと、フォルダと設定ファイルの自動生成ができなくなりました。

つまり、以前はtkfm.jar単体で動作していたのを、設定ファイルを事前に用意していない
と起動できなくなったということです。

実行ファイルの場所を変更する場合は、tkfmf2000.jarの入っているフォルダ(zip展開時
のフォルダ)ごと移動するようにしてください。

それ以外は特に変更していないので動作すると思います。多分。

その他の変更点としては、

1. 名前に2000となっているとおり、ツクール2000のみ対応です。MV, VXACEように出力す
   る機能は排除、というより使えなくしています。
   それらの機能を必要としている方はTKoolFacetileMaker2の方を使ってください。

2. Javaのバージョンが上がってます。TKoolFacetileMaker2ではバージョン1.8.0-101で
   したが、今回作成したのはJava1.8.0-121です。
   TKFM2が動いていたのでしたら多分動くと思いますが、動かなかったらJavaのアップデ
   ートをしてみてください。
